...
do
    ...
    RETRY=$((RETRY+1))
    echo "Elasticsearch not ready yet (HTTP ${HTTP_CODE}). Retry ${RETRY}/${MAX_RETRIES}..."
    sleep 5
done

if [ "$RETRY" -eq "$MAX_RETRIES" ]; then
    echo "ERROR: Elasticsearch did not become available after $((MAX_RETRIES * 5)) seconds."
    exit 1
fi
...
bash /opt/workshops/elastic-llm.sh -m gpt-5.5 -k true -d true 
...
curl -u sdg:changeme -X POST "http://localhost:30002/internal/product_doc_base/install" -H "kbn-xsrf: true" -H "x-elastic-internal-origin: Kibana" -H "Content-Type: application/json" -d '{"resourceType":"security_labs"}'
