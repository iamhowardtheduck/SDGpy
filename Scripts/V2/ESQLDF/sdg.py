...
# ─────────────────────────────────────────────────────────────────────────────
# Shared identity layer (identity.py, canonical home = this repo)
#
# Lets a workload emit a COHERENT employee per document: the internal client
# IP, the person's name, email, department/team and 7-digit employee_id all
# describe ONE real employee from the HR roster, so live SDG events join to
# the roster (by IP) and to the transaction archive (by employee_id) exactly
# like the ESQL-DataFederation Parquet/CSV data does.
#
# A per-document "persona" is chosen once (memoised in the worker's state
# dict) and every employee_* field in that document reads from it. Set
# suspicious_rate on the identity fields of a workload to make its persona the
# shared attacker that owns 10.49.110.17. persona_slot lets one document carry
# two distinct people (e.g. email sender vs recipient).
# ─────────────────────────────────────────────────────────────────────────────
import os
...
def _load_identity():
    ...
    if not os.path.exists(path):
        return None
    spec = _ilu.spec_from_file_location("identity", path)
    mod = _ilu.module_from_spec(spec)
    spec.loader.exec_module(mod)
    if getattr(mod, "IDENTITY_VERSION", None) != "1":
        log.warning("identity.py present but IDENTITY_VERSION != '1'; "
                    "employee_* field types disabled")
        return None
    return mod
...
def _persona(field: Dict, state: Dict) -> Dict:
    """Return this document's employee for the requested persona_slot,
    choosing (and memoising) one on first access."""
    slot = str(field.get("persona_slot", "0"))
    key = f"__persona_{slot}__"
    p = state.get(key)
    if p is None:
        srate = float(field.get("suspicious_rate", 0) or 0)
        if srate > 0 and random.random() < srate:
            emp = IDENT.employee_for_ip(IDENT.SUSPICIOUS_IP)
        else:
            emp = random.randint(1, IDENT.NUM_EMPLOYEES)
        p = IDENT.employee(emp)
        state[key] = p
    return p


def _require_identity(field: Dict) -> bool:
    if IDENT is None:
        log.warning("field '%s' needs identity.py next to sdg.py — "
                    "returning empty string", field.get("name", "?"))
        return False
    return True


def gen_employee_ipv4(field: Dict, state: Dict):
    return _persona(field, state)["ip"] if _require_identity(field) else ""


def gen_employee_ip_octet(field: Dict, state: Dict):
    """One octet (1..4) of the persona's workstation IP, so the existing
    dotted-octet ingest pipeline keeps working while the assembled address
    now belongs to a real employee."""
    if not _require_identity(field):
        return 0
    idx = int(field.get("octet", 1))
    return int(_persona(field, state)["ip"].split(".")[idx - 1])


def gen_employee_first_name(field: Dict, state: Dict):
    return _persona(field, state)["first_name"] if _require_identity(field) else ""


def gen_employee_last_name(field: Dict, state: Dict):
    return _persona(field, state)["last_name"] if _require_identity(field) else ""


def gen_employee_full_name(field: Dict, state: Dict):
    if not _require_identity(field):
        return ""
    p = _persona(field, state)
    return f'{p["first_name"]} {p["last_name"]}'


def gen_employee_email(field: Dict, state: Dict):
    if not _require_identity(field):
        return ""
    p = _persona(field, state)
    domain = field.get("domain", "example.com")
    return f'{p["first_name"].lower()}.{p["last_name"].lower()}@{domain}'


def gen_employee_username(field: Dict, state: Dict):
    return _persona(field, state)["username"] if _require_identity(field) else ""


def gen_employee_id(field: Dict, state: Dict):
    return _persona(field, state)["employee_id"] if _require_identity(field) else ""


def gen_employee_department(field: Dict, state: Dict):
    return _persona(field, state)["department"] if _require_identity(field) else ""


def gen_employee_team(field: Dict, state: Dict):
    return _persona(field, state)["team"] if _require_identity(field) else ""


# These take (field, state); routed specially in generate_value below.
IDENTITY_GENERATORS: Dict[str, Any] = {
    "employee_ipv4": gen_employee_ipv4,
    "employee_ip_octet": gen_employee_ip_octet,
    "employee_first_name": gen_employee_first_name,
    "employee_last_name": gen_employee_last_name,
    "employee_full_name": gen_employee_full_name,
    "employee_email": gen_employee_email,
    "employee_username": gen_employee_username,
    "employee_id": gen_employee_id,
    "employee_department": gen_employee_department,
    "employee_team": gen_employee_team,
}
...
def generate_value(field: Dict, state: Optional[Dict] = None) -> Any:
    ...
    if field_type in IDENTITY_GENERATORS:
        if state is None:
            state = {}
        return IDENTITY_GENERATORS[field_type](field, state)
...
def build_document(fields: List[Dict], seq_state: Dict) -> Dict:
    ...
    # Fresh persona(s) per document; sequence counters in seq_state persist.
    for k in [k for k in seq_state if k.startswith("__persona_")]:
        del seq_state[k]
    doc: Dict = {}
    for field in fields:
        key   = field["name"]
...
def main():
    ...
    # Warm the identity IP assignment single-threaded so worker threads never
    # race on first use.
